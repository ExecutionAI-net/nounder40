...
// Upload/replace the package image (shown to students when buying)
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: pkg } = await admin.from('packages').select('id, image_url').eq('id', id).eq('school_id', auth.profile.school_id).single()
  ...
  const path = `packages/${id}-${Date.now()}.${ext}`
