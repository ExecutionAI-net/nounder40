...
// Upload/replace the subscription image (shown to students when buying)
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: sub } = await admin.from('subscriptions_catalog').select('id, image_url').eq('id', id).eq('school_id', auth.profile.school_id).single()
  ...
  const path = `subscriptions/${id}-${Date.now()}.${ext}`
  ...
  const { error } = await admin.from('subscriptions_catalog').update({ image_url: pub.publicUrl }).eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  // best-effort cleanup of the previous file
  if (sub.image_url) {
    const old = sub.image_url.split('/school-assets/')[1]
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await admin.from('subscriptions_catalog').update({ image_url: null }).eq('id', id)
