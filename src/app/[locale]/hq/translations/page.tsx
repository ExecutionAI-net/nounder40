    ...
    <div className="p-6 max-w-screen-xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Manage all platform translations across 5 languages. Run{' '}
          <code className="bg-gray-100 px-1 rounded text-xs">npm run sync-translations</code>{' '}
          after adding new keys in code.
        </p>
      </div>
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        />
        <span className="text-sm text-gray-400">{filtered.length} keys</span>
      </div>

      {loading ? (
        <div className="text-sm text-gray-400 py-12 text-center">Loading translations...</div>
      ) : (
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={LOCALES.length + 2} className="px-4 py-12 text-center text-gray-400 text-sm">
                      {search ? 'No keys match your search.' : 'No translation keys yet. Run npm run sync-translations to populate.'}
                    </td>
                  </tr>
                )}
              </tbody>
