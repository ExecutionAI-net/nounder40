...
4. ~~The Medium/Low findings (M-1 through M-8 and the Low/cosmetic list) were **not** addressed in this round — still open, still accurately described above.~~ **Addressed in a follow-up round the same day — see below.**

---

## Medium/Low fix status (2026-09-07, follow-up round)

All Medium findings except M-8 (a documentation fix, done separately — see `DROP_IN_BOOKING.md`) and 6 of the Low/cosmetic items were fixed by 3 more parallel agents, same methodology as the Critical/High round: isolated worktrees, coordinator-verified (full backend `pytest` in a one-off container, diff review, dry-run merge checks for the two branches that both touched `frontend/messages/*.json`), merged one at a time.

| Findings | PR | What actually changed |
|---|---|---|
| M-1, M-2, M-3, M-4, M-5, School L-3 | [#74](https://github.com/ExecutionAI-net/nounder40/pull/74) | `PackageSerializer` now rejects `credits <= 0` and negative `price`. `SchoolSerializer` now enforces `0-100` on `platform_fee_percentage`/`shop_commission_percentage`. `SchoolRoomSerializer` now requires `capacity >= 1`. `CreditGrantView` now returns a clean `400` for amounts above the underlying `DecimalField`'s max (`99999.9`) instead of crashing with a `500`. `DiscountCodeSerializer` now enforces `0 < value <= 100` for percentage codes. |
| M-6, M-7, HQ L-3, Teacher Compensation hardcoded strings | [#75](https://github.com/ExecutionAI-net/nounder40/pull/75) | School Team invite emails now thread a resolved locale (explicit request locale → `School.language` → English) through all 3 call sites, matching the Teacher-invite path. `middleware.ts`'s locale-cookie override — which silently redirected an explicit URL locale (e.g. a shared `/it/...` link) to whatever the visitor's cookie said — is removed; next-intl's own routing already leaves a present, valid locale prefix untouched. Login page's hardcoded Italian email placeholder is now a real i18n key. 8 hardcoded English strings on the Teacher Compensation page are now translated in all 5 locales. |
| Teacher status-button colors, Performance breakdown title, Attendance past-lessons, Library blank titles, HQ role label, Students Followed KPI | [#76](https://github.com/ExecutionAI-net/nounder40/pull/76) | Attendance-status buttons/badges now fall back to a neutral gray instead of white-on-white when a status has no color. Performance page's breakdown section no longer permanently shows "No Data" as its heading. Teacher Attendance list gained a "Past" section — past/unmarked lessons no longer just disappear. Library items with blank titles now show a localized "Untitled" fallback. HQ dashboard's role badge now shows the DB role's actual `label` (already returned by `/hq/permissions/mine/`) instead of a raw key like `QA-CUSTOM-TEST` for custom roles. Bonus fix: "Students Followed" KPI now counts distinct students instead of attendance rows (was overcounting any teacher with repeat students — normal for a dance school). |
| M-8 | [#72](https://github.com/ExecutionAI-net/nounder40/pull/72) | `CLAUDE.md`/`DROP_IN_BOOKING.md` corrected — drop-in booking is fully implemented and live, not an unimplemented proposal; the docs had drifted from reality. |
| sync | [#77](https://github.com/ExecutionAI-net/nounder40/pull/77) | `develop` → `main` (bundled with unrelated concurrent work from another session). |

**Not fixed, by design or investigation:**
- **School L-2** (Package "Duplicate" swallowing a validation error) — investigated directly: `Package` has no `UniqueConstraint`, and Duplicate already shares the exact same error-handling code path as New Package (confirmed via a live Django-shell check of `PackageSerializer.validate()`'s actual error paths). Not reproducible as described; left unchanged.
- **HQ L-1** (custom-role dashboard badge showing the raw role key) — this was the same underlying issue as the HQ dashboard fix above and was fixed as part of #76.
- **HQ L-2** (HQ team-invite emails hardcoded to English) — no "HQ language" concept exists to localize against; a conscious product-scope gap noted in the original report, not treated as a bug, intentionally left as-is.
- **Teacher**: no password-change UI in the Teacher panel (may be intentional, not touched).
- **Student**: Danza Clásica Barcelona's package "(copy)"/"(copy) (copy)" duplicates (data-hygiene issue, needs a manual DB cleanup, not a code fix); the browser-autofill artifact (traced to the shared-browser QA setup, not an app bug).

**Everything from the original Critical/High "Still recommended" list remains open**: Gmail reconnect + 5 pending email checks, cancelling the orphaned Stripe test-mode subscription, and a live end-to-end re-verification of the Stripe purchase flows against `dev.danzaclassicanounder40.com`.
