...
// Upload/replace the course image (shown to students when booking)
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: course } = await admin.from('courses').select('id, image_url').eq('id', id).eq('school_id', auth.profile.school_id).single()
  ...
  const { error } = await admin.from('courses').update({ image_url: pub.publicUrl }).eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  // best-effort cleanup of the previous file
  if (course.image_url) {
    const old = course.image_url.split('/school-assets/')[1]
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await admin.from('courses').update({ image_url: null }).eq('id', id)
