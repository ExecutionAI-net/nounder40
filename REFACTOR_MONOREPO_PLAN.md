...
**Durum:** ✅ Fazlar 0-7 tamamlandı; `feature/monorepo-django` → `develop`'a merge edildi. Faz 8'in **prod compose/nginx/Dockerfile yarısı bitti** (deploy hariç, EC2 erişimi yok) ve **ETL script'i yazıldı + uçtan uca doğrulandı** (`backend/core/management/commands/etl_from_supabase.py`, runbook: `docs/etl/`); **kalan:** canlı Supabase prod verisiyle gerçek kesme (cutover) + storage bucket byte transferi + gerçek EC2 deploy — hepsi gerçek prod/altyapı erişimi gerektiriyor.
**Branch:** `develop` (monorepo artık ana branch'te) — 50+ commit, hepsi origin'de
...
- **Faz 8** — ETL + prod. **Prod compose/nginx/Dockerfile: ✅** — `docker-compose.prod.yml` (yeni), `nginx/nginx-prod.conf` (TLS, HSTS, rate limit), `.env.prod.example`, `Makefile` prod hedefleri (`up-prod` vb.), ve `backend/Dockerfile`'daki gerçek bir prod-kırıcı hata düzeltildi: CMD `gunicorn config.wsgi:application` kullanıyordu — WSGI, Channels WebSocket trafiğini (chat/calendar realtime) hiç taşıyamaz, prod'da sessizce kırılırdı. `daphne config.asgi:application`'a geçirildi. Hepsi lokal olarak uçtan uca canlı doğrulandı (fresh DB'de migration, HTTP→HTTPS redirect, WS upgrade nginx→daphne'ye ulaşıyor, güvenlik header'ları — nginx/Next.js/Django üçünün üst üste bindiği yerler bulunup tekilleştirildi). **ETL script: ✅ yazıldı + uçtan uca doğrulandı** — `etl_from_supabase` management command'ı (introspection-driven kolon-kesişimli kopyalama; tek özel-durumlar: `auth.users`+`profiles`→`accounts_user` join + bcrypt şifre göçü, `hq_members.id`→`user_id`, `schools.user_id`→`owner_id`, `_id`-sonek düşürme fallback'i, composite→auto id, target-only NOT NULL alanlar için üretilen default, döngüsel FK için `SET CONSTRAINTS ALL DEFERRED`). Doğrulama: (a) 16 birim testi (`core/tests/test_etl_mapping.py`, reponun ilk otomatik testleri — pytest), (b) gerçek `supabase/migrations/*.sql` bir throwaway Postgres'e replay edilip bağlı bir örnek graf seed edildi, ETL taze Django şemasına çalıştırıldı, 58 satır/47 tablo commit edildi ve zorlu kolonlar (şifre eşlemesi, dönüştürülen FK'ler, JSONB, UUID[], üretilen ical_token, FK bütünlüğü) DB'de doğrulandı. Fixtures + runbook: `docs/etl/`. Şifre göçü için `bcrypt` + `PASSWORD_HASHERS` eklendi. **Kalan (canlı prod erişimi gerekiyor):** gerçek Supabase prod DSN'iyle kesme, storage bucket byte transferi (runbook'ta yordam belgeli — DB ETL referansları koruyor, byte'lar taşınmıyor). **Gerçek EC2 deploy (domain/DNS/certbot): ⏳** — ayrı devops hattı ilerletiyor (bkz. `.github/workflows`, `docker-compose.run.yml`).
