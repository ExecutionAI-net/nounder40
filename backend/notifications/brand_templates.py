...
# R2-M20c: la copia di student.account_deleted dà per scontato che sia
# stata l'allieva a cancellarsi. Quando è la scuola a eliminare il profilo
# (students/school_views.py) quel testo è semplicemente falso: chiave
# separata, stessa struttura, stesse cinque lingue.
_t("student.account_deleted_by_school",
   it=('🕊️ Il tuo account è stato eliminato da {{school_name}}',
       'Ciao {{student_first_name}} 🌸\n\nti scriviamo per informarti che {{school_name}} ha eliminato il tuo account su Danza Classica No Under 40: il tuo profilo, le prenotazioni e i documenti sono stati cancellati.\n\nNon è stata una tua richiesta: se pensi che ci sia stato un errore, scrivi direttamente a {{school_name}}.\n\nSe un giorno vorrai tornare a ballare con noi, ti basterà creare un nuovo account. ✨\n\n[🩰 Crea un nuovo account|{{register_url}}]\n\nLa sbarra resta al suo posto, e i sogni non hanno età. 🩰\n\n' + SIGN['it']),
   en=('🕊️ Your account has been deleted by {{school_name}}',
       'Hi {{student_first_name}} 🌸\n\nwe are writing to let you know that {{school_name}} has deleted your Danza Classica No Under 40 account: your profile, bookings and documents have been removed.\n\nThis was not a request of yours: if you think it was a mistake, please contact {{school_name}} directly.\n\nIf one day you want to dance with us again, you will just need to create a new account. ✨\n\n[🩰 Create a new account|{{register_url}}]\n\nThe barre stays right where it was, and dreams have no age. 🩰\n\n' + SIGN['en']),
   es=('🕊️ {{school_name}} ha eliminado tu cuenta',
       'Hola {{student_first_name}} 🌸\n\nte escribimos para informarte de que {{school_name}} ha eliminado tu cuenta de Danza Classica No Under 40: tu perfil, tus reservas y tus documentos se han cancelado.\n\nNo ha sido una petición tuya: si crees que se trata de un error, escribe directamente a {{school_name}}.\n\nSi algún día quieres volver a bailar con nosotras, solo tendrás que crear una cuenta nueva. ✨\n\n[🩰 Crear una cuenta nueva|{{register_url}}]\n\nLa barra sigue en su sitio, y los sueños no tienen edad. 🩰\n\n' + SIGN['es']),
   fr=('🕊️ Votre compte a été supprimé par {{school_name}}',
       "Bonjour {{student_first_name}} 🌸\n\nnous vous informons que {{school_name}} a supprimé votre compte Danza Classica No Under 40 : votre profil, vos réservations et vos documents ont été effacés.\n\nCe n'est pas vous qui l'avez demandé : si vous pensez qu'il s'agit d'une erreur, contactez directement {{school_name}}.\n\nSi un jour vous souhaitez redanser avec nous, il vous suffira de créer un nouveau compte. ✨\n\n[🩰 Créer un nouveau compte|{{register_url}}]\n\nLa barre reste à sa place, et les rêves n'ont pas d'âge. 🩰\n\n" + SIGN['fr']),
   de=('🕊️ Dein Konto wurde von {{school_name}} gelöscht',
       'Hallo {{student_first_name}} 🌸\n\nwir informieren dich, dass {{school_name}} dein Konto bei Danza Classica No Under 40 gelöscht hat: dein Profil, deine Buchungen und deine Dokumente wurden entfernt.\n\nDas war keine Anfrage von dir: wenn du glaubst, dass es ein Irrtum war, wende dich bitte direkt an {{school_name}}.\n\nWenn du eines Tages wieder mit uns tanzen möchtest, musst du nur ein neues Konto anlegen. ✨\n\n[🩰 Neues Konto anlegen|{{register_url}}]\n\nDie Stange bleibt, wo sie ist, und Träume haben kein Alter. 🩰\n\n' + SIGN['de']))

# R2-M14b: la pagina di rientro del Negozio prometteva un'email di conferma
# ordine che non esisteva. Questa è quella email: parte da
# commerce.services.notify_shop_order(), sempre su transaction.on_commit.
_t("student.shop_order_confirmed",
   it=('🛍️ Ordine confermato #{{order_number}} — {{order_total}}',
       "Ciao {{student_first_name}} 🌸\n\ngrazie! Abbiamo ricevuto il tuo pagamento e il tuo ordine è confermato.\n\n🧾 Ordine #{{order_number}} del {{order_date}}\n🛍️ {{order_items}}\n💶 Articoli: {{order_subtotal}}\n🏷️ Sconto: {{order_discount}}\n📦 Spedizione: {{order_shipping}}\n💳 Totale pagato: {{order_total}}\n\nTi avviseremo appena l'ordine parte. Per qualsiasi domanda puoi rispondere a questa email.\n\n[🧾 Vedi i miei acquisti|{{orders_url}}]\n\n" + SIGN['it']),
   en=('🛍️ Order confirmed #{{order_number}} — {{order_total}}',
       'Hi {{student_first_name}} 🌸\n\nthank you! We received your payment and your order is confirmed.\n\n🧾 Order #{{order_number}} of {{order_date}}\n🛍️ {{order_items}}\n💶 Items: {{order_subtotal}}\n🏷️ Discount: {{order_discount}}\n📦 Shipping: {{order_shipping}}\n💳 Total paid: {{order_total}}\n\nWe will let you know as soon as it ships. For any question, just reply to this email.\n\n[🧾 See my purchases|{{orders_url}}]\n\n' + SIGN['en']),
   es=('🛍️ Pedido confirmado #{{order_number}} — {{order_total}}',
       'Hola {{student_first_name}} 🌸\n\n¡gracias! Hemos recibido tu pago y tu pedido está confirmado.\n\n🧾 Pedido #{{order_number}} del {{order_date}}\n🛍️ {{order_items}}\n💶 Artículos: {{order_subtotal}}\n🏷️ Descuento: {{order_discount}}\n📦 Envío: {{order_shipping}}\n💳 Total pagado: {{order_total}}\n\nTe avisaremos en cuanto salga. Para cualquier duda, responde a este email.\n\n[🧾 Ver mis compras|{{orders_url}}]\n\n' + SIGN['es']),
   fr=('🛍️ Commande confirmée #{{order_number}} — {{order_total}}',
       "Bonjour {{student_first_name}} 🌸\n\nmerci ! Nous avons reçu votre paiement et votre commande est confirmée.\n\n🧾 Commande #{{order_number}} du {{order_date}}\n🛍️ {{order_items}}\n💶 Articles : {{order_subtotal}}\n🏷️ Remise : {{order_discount}}\n📦 Livraison : {{order_shipping}}\n💳 Total payé : {{order_total}}\n\nNous vous préviendrons dès l'expédition. Pour toute question, répondez simplement à cet email.\n\n[🧾 Voir mes achats|{{orders_url}}]\n\n" + SIGN['fr']),
